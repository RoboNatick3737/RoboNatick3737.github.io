...
<div class="container marketing">
  ...
  <div>
	...
	<script>
		...
		<?php
			$count = 2;
			$result = mysqli_query($con,"SELECT * FROM USERS WHERE D411='1'");
			while($row = mysqli_fetch_array($result)) {
				
				$name = $row[\'FIRST_NAME\'];
				$lat = $row[\'LATITUDE\'];
				$lng = $row[\'LONGITUDE\'];
				echo "var marker" . $count . " = L.marker([" . doubleval($lat) . ", " . doubleval($lng) . "]).addTo(map).bindPopup(\"<b>" . $name . "</b><br>" . doubleval($lat) . "," . doubleval($lng) . "\").openPopup();\n\t\t";
